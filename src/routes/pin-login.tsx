...
export function PinLoginPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!ok) { setSubmitting(false); setPin(''); return; }
    const restored = await restoreFromSecureStorage();
    setSubmitting(false);
    if (restored) { navigate('/dashboard', { replace: true }); }
    else { await resetAll(); navigate('/login', { replace: true }); }
  };
      ...
      <div className="relative z-10 w-full max-w-sm rounded-2xl border border-admin-900/30 bg-midnight-900/80 p-8 shadow-2xl shadow-black/60 backdrop-blur-xl">
        ...
        <div className="mb-6 flex flex-col items-center text-center">
          <div className={cn(
            ...
            {isLocked ? <Lock className="h-7 w-7 text-white" /> : <Shield className="h-7 w-7 text-white" />}
          </div>
          <h1 className="font-display text-2xl font-bold text-gold-100">
            {isLocked ? t('inactive') : t('pin_title')}
          </h1>
          {lastEmail && <p className="mt-1 text-xs text-gold-400">{lastEmail}</p>}
          {!isLocked && (
            <p className="mt-2 text-sm text-gold-700">
              {PIN_LENGTH} {t('pin_subtitle')}
            </p>
          )}
        </div>
        ...
        {isLocked ? (
          <div className="text-center">
            ...
            <p className="mt-3 text-sm text-gold-700">Blokirovka tugashini kuting</p>
          </div>
        ) : (
          <div>
            ...
            <button type="button" onClick={handleSubmit}
              ...
              {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : t('pin_btn')}
            </button>
          </div>
        )}
