...
export function SongsPage() {
  ...
  const FILTERS: { key: FilterKey; label: string }[] = [
    { key: 'all',         label: t('songs_filter_all') },
  ...
  const loadSongs = async (silent = false) => {
    ...
    if (error) { setError(error.message); setSongs([]); }
    else {
      const mapped: AdminSongRow[] = (data ?? []).map((row: any) => ({
        id: row.id, user_id: row.user_id, title: row.title, prompt: row.prompt,
        audio_url: row.audio_url, duration_seconds: row.duration_seconds,
        status: row.status, is_published: row.is_published,
        is_flagged: row.is_flagged, flag_reason: row.flag_reason,
        flagged_at: row.flagged_at, moderation_note: row.moderation_note,
  ...
  const filtered = useMemo(() => {
    ...
    return songs.filter((s) => {
      ...
      if (!term) return true;
      const haystack = [s.title, s.prompt, s.author_email, s.author_username, s.author_full_name]
  ...
  return (
    <div className="p-6 lg:p-8">
      <PageHeader
        icon={Music}
        title={t('songs_title')}
        subtitle={`${t('all')}: ${songs.length} · Flag: ${counts.flagged} · Unpublished: ${counts.unpublished}`}
        action={
          <button type="button" onClick={() => loadSongs(true)} disabled={refreshing}
      ...
      <div className="overflow-hidden rounded-xl border border-gold-900/30 bg-midnight-900/40">
        {loading ? (
          ...
          <div className="py-20 text-center text-sm text-gold-700">
            {search ? t('songs_search_empty') : t('songs_empty')}
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gold-900/30 bg-midnight-950/60 text-left text-[10px] uppercase tracking-wider text-gold-700">
                  ...
                  <th className="px-4 py-3 font-semibold">{t('songs_title')}</th>
      ...
      {filtered.length > 0 && (
        <div className="mt-3 text-xs text-gold-700">
          {filtered.length} {t('songs_title').toLowerCase()}
          {filtered.length < songs.length && ` (${songs.length} dan)`}
        </div>
      )}

      {selected && (
        <SongDetailsModal song={selected} open={!!selected}
          onClose={() => setSelected(null)}
          onUpdated={handleSongUpdated} onDeleted={handleSongDeleted} />
      )}
    </div>
  );
}

function SongRow({ song, isPlaying, onTogglePlay, onClick }: {
  song: AdminSongRow; isPlaying: boolean;
  onTogglePlay: () => void; onClick: () => void;
}) {
  const statusVariant = (() => {
    switch (song.status) {
  ...
  return (
    <tr onClick={onClick}
      ...
      <td className="px-3 py-3" onClick={(e) => e.stopPropagation()}>
        <button type="button" onClick={onTogglePlay} disabled={!song.audio_url}
          className={cn('grid h-8 w-8 place-items-center rounded-full transition-colors disabled:opacity-30',
            isPlaying ? 'bg-admin-500 text-white' : 'bg-gold-500/15 text-gold-300 hover:bg-gold-500/25')}>
      ...
      <td className="px-4 py-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            {song.is_flagged && <Flag className="h-3 w-3 shrink-0 text-admin-400" />}
            {song.is_published === false && <EyeOff className="h-3 w-3 shrink-0 text-amber-400" />}
          </div>
          {song.prompt && (
            <div className="mt-0.5 line-clamp-1 text-[11px] text-gold-700">{song.prompt}</div>
          )}
        </div>
      </td>
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            <div className="truncate text-xs font-medium text-gold-100">{song.author_full_name || '—'}</div>
      ...
      <td className="px-4 py-3"><Badge variant={statusVariant}>{song.status || '—'}</Badge></td>
      <td className="px-4 py-3 text-xs text-gold-300/80">{formatDuration(song.duration_seconds)}</td>
      <td className="px-4 py-3 text-xs text-gold-700">{timeAgo(song.created_at)}</td>
