...
const FILTERS: { key: FilterKey; label: string; color: string }[] = [
  { key: 'all',    label: 'Hammasi',    color: 'text-gold-300' },
  { key: 'active', label: 'Faol',       color: 'text-emerald-300' },
  { key: 'admin',  label: 'Adminlar',   color: 'text-admin-300' },
  { key: 'banned', label: 'Banlangan',  color: 'text-amber-300' },
];
...
export function UsersPage() {
  ...
  // Filtrlash + qidiruv (client-side, 500 user'gacha tez ishlaydi)
  const filtered = useMemo(() => {
    ...
    return users.filter((u) => {
      ...
      // Search
      if (!term) return true;
      const haystack = [u.email, u.username, u.full_name]
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Email yoki username bo'yicha qidiruv..."
      ...
      <div className="overflow-hidden rounded-xl border border-gold-900/30 bg-midnight-900/40">
        {loading ? (
          ...
          <div className="py-20 text-center text-sm text-gold-700">
            {search ? "Qidiruv bo'yicha hech narsa topilmadi" : "Foydalanuvchilar yoʻq"}
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gold-900/30 bg-midnight-950/60 text-left text-[10px] uppercase tracking-wider text-gold-700">
                  ...
                  <th className="px-4 py-3 font-semibold">Oxirgi kirish</th>
                  <th className="px-4 py-3 font-semibold">Roʻyxatdan oʻtgan</th>
      ...
      {filtered.length > 0 && (
        <div className="mt-3 text-xs text-gold-700">
          {filtered.length} ta foydalanuvchi koʻrsatildi
    ...
    >
      ...
      <td className="px-4 py-3">
        <div className="flex flex-wrap gap-1">
          ...
          {user.is_banned && (
            <Badge variant="amber" icon={<Ban className="h-3 w-3" />}>
              Banned
            </Badge>
          )}
